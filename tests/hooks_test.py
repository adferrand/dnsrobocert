...
from dnsrobocert.core import hooks
...
def fake_env(tmp_path):
    ...
    try:
        ...
        os.environ['CERTBOT_DOMAIN'] = LINEAGE
        os.environ['RENEWED_LINEAGE'] = str(live_path)
        yield {
            'live': live_path,
...
def test_pfx(_autorestart, _autocmd, _fix_permissions, fake_env, fake_config):
    ...
    subject = issuer = x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, u'example.com')])
...
def test_autocmd(check_call, _exists, _autorestart, _pfx_export, _fix_permissions, fake_config):
    hooks.deploy(config.load(fake_config), LINEAGE)
...
@patch('dnsrobocert.core.hooks.os.path.exists')
@patch('dnsrobocert.core.hooks.utils.execute')
def test_autorestart(check_call, _exists, _autocmd, _pfx_export, _fix_permissions, fake_config):
    hooks.deploy(config.load(fake_config), LINEAGE)

    call_container1 = call(['docker', 'restart', 'container1'])
    call_container2 = call(['docker', 'restart', 'container2'])
    call_service1 = call(['docker', 'service', 'update', '--detach=false', '--force', 'service1'])
    call_service2 = call(['docker', 'service', 'update', '--detach=false', '--force', 'service2'])
    check_call.assert_has_calls([call_container1, call_container2, call_service1, call_service2])
